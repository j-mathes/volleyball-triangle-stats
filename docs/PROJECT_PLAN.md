...
## Goal
Build Triangle Stats as a TypeScript monorepo with a React web app, backed by an event-sourced domain layer that supports real-time updates, undo/redo, per-set and match aggregates, and JSON/CSV export.

## Product Scope (v1)
Included:
- Browser app with a shared domain package
...
- Web app (`apps/web`) renders UI and uses persistence adapters

## Delivery Phases

### Phase 1: Foundation
- Monorepo setup (`apps/web`, `packages/shared`)
...
- App entry placeholder for web
- Basic architecture/readme docs

Next major milestone:
- Make web app runnable and wire the first interactive triangle stat screen to shared engine
